...
public class Deal {
	...
	//more props
	public SimpleStringProperty dCityName;
	public SimpleStringProperty dMerchant;
	...
	public Deal(Connection conn, 
		...
		this.dCityName=new SimpleStringProperty(getCity(new TextField(location_ID.toString())));
	...
	public Deal(
		...
		if(tflocation_ID == null || tflocation_ID.getText().isEmpty()) {
			...
			this.dCityName=new SimpleStringProperty("unknown");
		}else if (!tflocation_ID.getText().isEmpty()){ 

			this.lID = new SimpleIntegerProperty(Integer.parseInt(tflocation_ID.getText()));
			this.dCityName=new SimpleStringProperty(getCity(tflocation_ID));
		}
	...
	public static ArrayList<Deal> retrieve(Connection conn,
		...
		try {
			...
			while (rs.next()) {
				...
				Deal deal = new Deal(Deal.conn,tfdid,tfexp,tfdes,tfqty,
						tforg,tfprc,tfstr,tfend,tfloc,tfcat,tfmid);
				
				
				deal.dCityName = new SimpleStringProperty(Deal.getCity(tfloc));
	...
	public static ArrayList<Deal> openDeals(TextField tflocation) 
...

	{
		...
		String selectSQL = "SELECT " +
				"deal_ID, " + 
				"to_char(expiration_date,'DD-MON-YYYY') as expiration_date, " +
				"description, quantity_limit, original_price, deal_price, " +
				"to_char(sale_start_time, 'DD-MON-YYYY') as sale_start_time, " +
				"to_char(sale_end_time, 'DD-MON-YYYY') as sale_end_time, " +
				"location_ID, category_ID, merchant_ID FROM Deal "+sqlWhere;
		
		boolean citySpecified = false;
		
		if(tflocation == null || tflocation.getText() == null || tflocation.getText().trim().isEmpty()) {
			
		}
		else {
			citySpecified = true;
			selectSQL+= " AND location_ID in (select location_ID from location " +
					"where city =\'"+tflocation.getText()+"\')";
		}
		...
		try {
			...
			while (rs.next()) {
				...
				if(citySpecified) {
					deal.dCityName = new SimpleStringProperty(tflocation.getText());
				}
	...
	public static String getCity(TextField loc) {
		// adding city name
		connect();
		
		String res = null;
		
		Integer locId = Integer.parseInt(loc.getText());
		
		PreparedStatement preparedStatement = null;
		 
		String selectSQL = "select city from location where location_id=\'"+locId+"\'";
