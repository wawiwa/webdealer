...
public class Review {
	...
	public static Double getAvgRating(TextField mid) {
		if(!connect()) AppUtil.console("Not able to connect to database!");
		Integer mid_val = Integer.parseInt(mid.getText());
		String selectSQL = "select avg(r.rating) as avg_rating from review r where r.deal_id in "+
				"( select d.deal_id from deal d where d.merchant_id=\'"+mid_val+"\')";
		...
		try {
			...
			if (rs.next()) { avg = rs.getDouble("avg_rating"); }
				...
				AppUtil.console("avg_rating : " + rs.getDouble("avg_rating"));
